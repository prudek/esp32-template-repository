...
- When creating branch names, do not use the `codex` prefix. Use the `prudek` prefix instead (for example: `prudek/esp32-template-bootstrap`).
- When creating pull requests, avoid `codex` naming in titles or branch references. Use neutral naming or `prudek` naming.
